...
**已修掉的問題：** (1) 缺少輕量模式——quick 模式已補上（單暫停點、不建 worktree、branch → 直改 → reviewer快掃 → PR），10 行 fix 不再跑 6 個 stage。(2) 參數配置收斂——model 別名收斂於 frontmatter 綁定，effort 配置則收斂於推論等級表；雖呼叫時需明確指明 effort（因 frontmatter 不再提供此屬性），但透過統一定義降低了 model 換代的維護稅。(3) 「文件 vs 執行」的可程式化部分——`scripts/wf-state.sh` 成為 state 唯一存取入口：schema 校驗 + 原子寫入殺掉手寫 JSON 腐壞，轉移表殺掉非法跳段，`--confirmed` 棘輪把「跳過暫停點」變成留下痕跡的蓄意動作；強制性跟著 `mode` 走（quick/jump 不套轉移表），quick 級升走單向 `upgrade`。
