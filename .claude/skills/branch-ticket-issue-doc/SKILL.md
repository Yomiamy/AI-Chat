...
description: 在 ticket-id-dev-prep 已選定或準備好開發工作區、且 Codex 需要從 advisor 解析的 brief、YouTrack ticket 細節與當前工作區程式碼脈絡建立或更新 docs/issues/<ticket-id>.md 時使用此 skill；只專注於記錄問題，不撰寫 spec 或程式碼。
---

# Branch Ticket Issue Doc

在 `ticket-id-dev-prep` 所選的開發工作區中使用此 skill。

目標：建立或更新 `docs/issues/<ticket-id>.md`，作為該分支的正規問題文件。

在此處不要建立 spec。在此處不要變更產品程式碼。

## 前置條件

1. 確認當前目錄是預期的開發工作區。
2. 確認當前分支包含 ticket id，或使用者明確提供 ticket id。
3. 優先使用 `branch-ticket-solution-advisor` 的解析 brief。
4. 若無解析 brief，讀取 YouTrack ticket，並只檢視足以記錄問題的程式碼脈絡。
5. 若 `docs/issues/` 不存在則建立。

若 `ticket-id-dev-prep` 選了不同的工作區，在寫入前切換過去。若所選策略是 `current-branch` 或 `current-worktree-new-branch`，允許在當前工作區寫入。

## 工作流程

1. 解析 ticket id。
2. 若存在則讀取既有的 `docs/issues/<ticket-id>.md`。
3. 若存在則讀取 `docs/issues/template.md`，並保留本地的文件風格。
4. 蒐集來源素材：
   - advisor 解析 brief
   - YouTrack 標題與描述
   - 相關欄位，例如 `State`、`Type`、`Priority`、`Subsystem`
   - 來自針對性檢視的當前 worktree 程式碼觀察
...
6. （Fallback）自行寫入或更新 `docs/issues/<ticket-id>.md`，依照 Issue Doc Template。
7. 區分 facts、inference 與 open questions。
...
只在段落真正不適用時才省略空段。寧用 `待確認`，也不要發明細節。

## 品質規則

- 問題文件必須在不讀完整 ticket 的情況下也能理解。
- 不要逐字複製冗長的 ticket 文字。
- 相關時保留確切的錯誤訊息、標籤、欄位名稱與使用者可見字串。
- 明確記下缺漏的重現資料。
- 若程式碼脈絡與 ticket 矛盾，記錄該不一致之處。
