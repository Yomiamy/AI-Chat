...
1. 確認當前目錄是 `ticket-id-dev-prep` 所選的開發工作區。
2. 從使用者輸入、分支名稱或既有 issue 文件解析 ticket id。
3. 先讀取 `docs/issues/<ticket-id>.md`。
4. 若 issue 文件缺失，停下並先執行 `branch-ticket-issue-doc`。
...
5. （Fallback）自行撰寫並建立或更新 `docs/issues/specs/<ticket-id>.md`，依照 Spec Template。
6. 讓 acceptance criteria 保持可測試。
...
1. `Spec`：建立或更新的路徑
2. `依據`：issue 文件路徑與已檢視的程式碼脈絡
3. `修正方向`：簡短摘要
4. `測試重點`
5. `阻塞`：僅當 open questions 阻塞實作時
6. `Next`：只有在沒有阻塞性問題殘留時，實作才能開始
